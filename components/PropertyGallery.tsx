...
const PropertyGallery = ({ images = [], video, title }: PropertyGalleryProps) => {
  ...
  const nextModalSlide = () => setModalIndex((prev) => (prev + 1) % total)
  const prevModalSlide = () => setModalIndex((prev) => (prev - 1 + total) % total)
  ...
  }, [modalIndex, isModalOpen, isMuted, modalMedia?.type])
        ...
        ReactDOM.createPortal(
          ...
          >
            <div className="relative w-full h-full flex items-center justify-center p-6">
              ...
              {/* Contador en modal */}
              <div className="absolute top-6 right-6 bg-black/60 text-white text-sm px-2 py-1 rounded-full">
                {modalIndex + 1}/{total}
              </div>
